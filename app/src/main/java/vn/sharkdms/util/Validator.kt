...
class Validator {
    companion object {
        ...
        private val PHONE_PATTERN: Pattern = Pattern.compile("^[0-9]{10,11}\$")
        ...
        private val ADDRESS_PATTERN: Pattern = Pattern.compile(
            "^[0-9a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳ" +
                    "ẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\\s\\W|_]+",
            Pattern.CASE_INSENSITIVE or Pattern.UNICODE_CASE)
        fun isValidName(name: String) = NAME_PATTERN.matcher(name.trim()).matches()
        fun isValidUsername(username: String) = USERNAME_PATTERN.matcher(username.trim()).matches()
        fun isValidPassword(password: String) = PASSWORD_PATTERN.matcher(password.trim()).matches()
        fun isValidEmail(email: String) = EMAIL_PATTERN.matcher(email.trim()).matches()
        fun isValidPhone(phone: String) = PHONE_PATTERN.matcher(phone.trim()).matches()
        fun isValidAddress(address: String) = ADDRESS_PATTERN.matcher(address.trim()).matches()
    }
}